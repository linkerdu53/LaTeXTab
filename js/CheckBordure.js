...
function CheckBordureRight(input) {
    //On ne fait pas pour la dernière ligne
...
function CheckBordureLeft(input) {
    //On ne fait pas pour la première colonne
    if (parseInt(input.parentNode.dataset.col) !== 1) {
        ...
        if (!inputGauche.parentElement.classList.contains("selected") || ( inputGauche.parentElement.classList.contains("selected") && input.classList.contains("selected") )) {
            if (!input.classList.contains("borderLeftOn")) {
                if (inputGauche.parentElement.classList.contains("borderRightOn")) {             
                    inputGauche.parentElement.style.borderRight = "";
                    inputGauche.parentElement.classList.remove("borderRightOn");
                }
            }
            else if (!inputGauche.parentElement.classList.contains("borderRightOn")) {
                inputGauche.parentElement.style.borderRight = "solid";
                inputGauche.parentElement.classList.add("borderRightOn");
            }
        }
    }
}
...
function CheckBordureTop(input) {
    //On ne fait pas pour la première ligne
